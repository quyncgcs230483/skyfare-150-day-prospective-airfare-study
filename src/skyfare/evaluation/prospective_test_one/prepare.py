#!/usr/bin/env python3
"""Build sealed prospective Test 1 frames from immutable daily raw files."""
...
TEST_START = pd.Timestamp("2026-07-29")
TEST_END = pd.Timestamp("2026-08-08")
TEST_END_EXCLUSIVE = TEST_END + pd.Timedelta(days=1)
TEST_2_START = pd.Timestamp("2026-08-09")
TRAIN_CUTOFF = pd.Timestamp("2026-07-28")
...
def _validate_test_daily_files(daily_paths: list[Path], temporal_sessions) -> dict[str, object]:
    ...
    if observed != expected:
        raise RuntimeError(f"Test 1 raw date coverage mismatch missing={sorted(expected - observed)}")
    rows = 0
    hashes: dict[str, str] = {}
...
def _build_raw_loader(
    ...
    if frozen["session_date"].max().normalize() != TRAIN_CUTOFF:
        raise RuntimeError("Frozen standard offers do not end at development cutoff")
    ...
    if set(incoming["session_date"].dt.strftime("%Y-%m-%d")) != {
        ...
        raise RuntimeError("Standard-tier Test 1 rows do not cover all 11 days")
    ...
    if combined["session_date"].max().normalize() != TEST_END:
        raise RuntimeError("Combined source does not end at Test 1 cutoff")
...
def _parity_report(rebuilt: pd.DataFrame, frozen: pd.DataFrame) -> dict[str, object]:
    old = rebuilt.loc[pd.to_datetime(rebuilt["session_date"]).le(TRAIN_CUTOFF)].copy()
...
def build(
    ...
    frozen_input_root = frozen_input_root.resolve()
    ...
    # QH availability varies by day. Latest Test 1 day has four observed airlines.
    ...
    frozen_offers_path = frozen_input_root / "standard_offers.parquet"
    class_dev = frozen_input_root / "classification_training_frame.parquet"
    reg_dev = frozen_input_root / "regression_training_frame.parquet"
    ...
    next_runtime.STANDARD_OFFERS_CACHE = work / "standard_test1_offers.parquet"
    ...
    if offers["feature_time"].max() >= TEST_2_START:
        raise RuntimeError("Test 2 row entered sealed offers")
    ...
    classification_test = exact.loc[
        ...
        & exact["label_time"].lt(TEST_2_START)
    ...
    if regression_test.empty or classification_test.empty:
        raise RuntimeError("Test 1 frame is empty")
    if regression_test["label_time"].max() >= TEST_2_START:
        raise RuntimeError("Test 2 regression label entered Test 1")
    if classification_test["label_time"].max() >= TEST_2_START:
        raise RuntimeError("Test 2 classification label entered Test 1")

    offers.to_parquet(output_root / "standard_offers.parquet", index=False, compression="zstd")
    regression_test.to_parquet(
        output_root / "regression_test_1_frame.parquet", index=False, compression="zstd"
    )
    classification_test.to_parquet(
        output_root / "classification_test_1_frame.parquet", index=False, compression="zstd"
    )
    observability.to_parquet(
        output_root / "classification_test_1_observability.parquet",
        index=False,
        compression="zstd",
    )
    ...
    artifact_names = [
        ...
        "regression_test_1_frame.parquet",
        "classification_test_1_frame.parquet",
        "classification_test_1_observability.parquet",
    ]
    report = {
        "status": "PASS",
        "protocol": "PROSPECTIVE_TEST_1_R2_RIGHT_CENSOR_SAFE",
        "development_cutoff": str(TRAIN_CUTOFF.date()),
        "test_1": [str(TEST_START.date()), str(TEST_END.date())],
        "test_2_preserved_from": str(TEST_2_START.date()),
        "raw_test_1": raw_audit,
        ...
        "test_2_rows_accessed": 0,
    }
    (output_root / "SEALED_TEST_1_MANIFEST_R2.json").write_text(
...
def main() -> None:
    ...
    build(args.frozen_input_root, args.output_root, args.trip_raw_root)
