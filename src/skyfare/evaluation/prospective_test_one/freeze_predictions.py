#!/usr/bin/env python3
"""Commit target-free Test 1 predictions before label evaluation."""
...
from skyfare.evaluation.prospective_test_one.contract import FINALISTS, QUANTILES, registry
...
def main() -> None:
    ...
    outputs["CLASSIFICATION"] = commit_root / "classification_test1_predictions.parquet"
    ...
    outputs["POINT"] = commit_root / "point_test1_predictions.parquet"
    ...
    outputs["DISTRIBUTION"] = commit_root / "distribution_test1_predictions.parquet"
    write_parquet_atomic(outputs["DISTRIBUTION"], distribution)

    ranking = _mean_aligned(_load_task("RANKING"), ["score"])
    outputs["RANKING"] = commit_root / "ranking_test1_predictions.parquet"
    ...
    write_json_atomic(OUTPUT_ROOT / "runtime" / "prediction_commit_test1.json", payload)
    print("[TEST1 PREDICTION COMMIT PASS] 4 tasks; labels unread", flush=True)
