#!/usr/bin/env python3
"""Build checksummed Test 1 result and compact evidence archives."""
...
def main() -> None:
    verification = json.loads((OUTPUT_ROOT / "VERIFICATION_TEST_1.json").read_text(encoding="utf-8"))
    ...
    with tarfile.open(temporary, "w:gz", compresslevel=6) as archive:
        _add_tree(archive, OUTPUT_ROOT, "artifacts/prospective_test_one")
        _add_tree(archive, Path(__file__).resolve().parent, "src/skyfare/evaluation/prospective_test_one")
    ...
    evidence_members = [
        OUTPUT_ROOT / "VERIFICATION_TEST_1.json",
        OUTPUT_ROOT / "analysis" / "TEST_1_REPORT.json",
        OUTPUT_ROOT / "runtime" / "pipeline_status_test1.json",
        OUTPUT_ROOT / "runtime" / "preflight_test1.json",
        OUTPUT_ROOT / "runtime" / "deployable_lock_test1.json",
        OUTPUT_ROOT / "runtime" / "prediction_commit_test1.json",
        OUTPUT_ROOT / "runtime" / "scheduler_test1.json",
        manifest_path,
        RELEASE_ROOT / f"{ARCHIVE_NAME}.sha256",
        OUTPUT_ROOT / "test_one.log",
    ]
    ...
    print(
        f"[TEST1 ARCHIVE PASS] result={archive_path.name} evidence={evidence_path.name} "
