#!/usr/bin/env python3
"""One-time evaluation of committed Test 1 predictions."""
...
from skyfare.evaluation.prospective_test_one.contract import BUY_WAIT_THRESHOLD, NOMINAL_FRICTION_VND, QUANTILES
...
def _join(path: str, labels: pd.DataFrame) -> pd.DataFrame:
    ...
    return prediction.merge(labels, on="row_key", how="inner", validate="one_to_one")
...
def main() -> None:
    commit_path = OUTPUT_ROOT / "runtime" / "prediction_commit_test1.json"
    ...
    if commit.get("preflight_sha256") != preflight_sha256():
        raise RuntimeError("Test 1 staged inputs changed after prediction commit")
    ...
    classification = _join("classification_test1_predictions.parquet", class_labels)
    point = _join("point_test1_predictions.parquet", reg_labels)
    distribution = _join("distribution_test1_predictions.parquet", reg_labels)
    ranking = _join("ranking_test1_predictions.parquet", reg_labels)
    ...
    report = {
        ...
        "test_2_access": False,
        "next_action": "RUN_TEST_2_WITH_SAME_DEPLOYABLE_LOCK" if decision == "PASS" else "RETAIN_INCUMBENT; DO_NOT_TUNE_ON_TEST_1; PRESERVE_TEST_2",
        "classification": {
            "model": class_metrics,
            "baseline_constant_dev_rate": class_base_metrics,
            "paired_day_bootstrap": _bootstrap(model_class_loss - base_class_loss, classification["feature_time"], 24101),
        },
        "point": {
            "model": point_result,
        ...
        "distribution": {
            ...
            "paired_day_bootstrap": _bootstrap(model_dist_loss - base_dist_loss, distribution["label_time"], 24103),
        },
        ...
        "buy_wait": {
            ...
            "paired_day_bootstrap_vs_buy_all": _bootstrap(policy_regret - buy_regret, classification["feature_time"], 24104),
            "threshold": BUY_WAIT_THRESHOLD,
            "friction_vnd": NOMINAL_FRICTION_VND,
        },
        "gates": gates,
    }
    write_json_atomic(OUTPUT_ROOT / "analysis" / "TEST_1_REPORT.json", report)
    print(f"[TEST1 EVALUATION {decision}] gates={sum(gates.values())}/{len(gates)}", flush=True)
