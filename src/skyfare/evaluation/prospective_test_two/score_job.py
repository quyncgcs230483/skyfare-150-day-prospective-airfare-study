#!/usr/bin/env python3
"""Refit one frozen V24 finalist and score target-free Test 2 features."""
...
from skyfare.evaluation.prospective_test_two.contract import QUANTILES, registry
from skyfare.evaluation.prospective_test_two.models import fit_predict
...
def validate_job(raw: dict[str, object]) -> dict[str, object]:
    ...
    if identifier not in expected or raw != expected[identifier]:
        raise RuntimeError(f"job is not in immutable Test 2 registry: {identifier}")
    return raw
...
def main() -> None:
    ...
    if artifact_complete(str(job["job_id"])) and not cli.force:
        print(f"[TEST2 JOB SKIP] {job['job_id']}", flush=True)
        return
    ...
    try:
        ...
        print(
            f"[TEST2 JOB START] {job['job_id']} fit={len(fit):,} head={len(head):,} "
        ...
        print(
            f"[TEST2 JOB PASS] {job['job_id']} rows={len(artifact):,} "
        ...
        print(f"[TEST2 JOB FAIL] {job['job_id']} {type(error).__name__}: {error}", file=sys.stderr, flush=True)
        raise
